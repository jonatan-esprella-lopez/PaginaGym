...
const trainingData = [
  {
    ...
    description: 'Este entrenamiento está dedicado a descubrir todo lo que necesitas saber sobre tu cuerpo, su funcionamiento y el dominio de cada parte, entenderás las rutinas de entrenamiento en este programa.',
...
const TrainingList = () => {
  return (
    <section className="training-list">
      <h2>Programas de entrenamiento <strong>Básico</strong></h2>
      <p>Inicia tus rutinas y obtén tus objetivos ya!!</p>

      {trainingData.map(training => (
        <TrainingItem key={training.id} title={training.title} description={training.description} tags={training.tags} />
      ))}
    </section>
  );
};
